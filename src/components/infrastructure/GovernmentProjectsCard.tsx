    ...
    return (
        <Card className="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border-violet-200/50 dark:border-violet-800/50">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-xl text-black dark:text-slate-300 font-semibold">Government Projects</CardTitle>
                <Building2 className="h-6 w-6 text-violet-600 dark:text-violet-400" />
            </CardHeader>
            <CardContent>
                <div className="space-y-4">
                    {[...(data?.big_projects || []), ...(data?.small_projects || [])].map((project) => (
                        <div key={project.id} className="border-b border-violet-200/30 dark:border-violet-800/30 pb-3">
                            <div className="flex justify-between items-center mb-2">
                                <h3 className="text-black dark:text-slate-300 font-medium text-lg">{project.name}</h3>
                                <span className={`flex items-center gap-1 px-2 py-1 rounded-full text-sm ${getStatusColor(project.status)}`}>
                    ...
                    {(!data?.big_projects?.length && !data?.small_projects?.length) && (
                        <div className="text-center py-6">
                            ...
                            <p className="text-gray-400 dark:text-gray-500 text-xs mt-1">Check back later for updates</p>
                        </div>
                    )}
                </div>
            </CardContent>
